...
export function DeployPanel({
  ...
  const { points, deployCount, boostActiveUntil } = hubStats;
  const previewDeploy =
    "previewDeploy" in hubStats ? hubStats.previewDeploy : undefined;
  ...
  const handleDeploy = () => {
    ...
    if (isPreviewMode() && previewDeploy) {
      previewDeploy();
      setDeployedToken("0xcccccccccccccccccccccccccccccccccccccccc");
      setPreviewDone(true);
      ...
      {(previewDone || (isSuccess && hash)) && (
        <div className="uni-card-inset flex flex-col gap-2 p-4 text-center">
          <p className="uni-label text-[var(--uni-success)]">
            Token deployed{previewDone ? " (preview)" : ""}
          </p>
          {lastEarned != null && (
