...
import { previewBadgesForAddress, previewRankForAddress } from "@/lib/previewMocks";
...
export function useBadgeStatus() {
  ...
  const enabled =
    Boolean(address) && isBadgeContractConfigured && !isPreviewMode();
  ...
  if (isPreviewMode() && address) {
    return {
      badges: previewBadgesForAddress(address),
      refresh: async () => {},
      isLoading: false,
      isConfigured: true,
      userRank: previewRankForAddress(address),
      milestoneMintedCount: 2n,
    };
  }
