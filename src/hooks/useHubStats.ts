...
export function useHubStats() {
  ...
  const enabled = Boolean(address) && isContractConfigured && !isPreviewMode();
  ...
  useWatchContractEvent({
    ...
    onLogs(logs) {
      if (
        logs.some(
          (log) => onMine(log.args.referrer) || onMine(log.args.referee),
        )
      ) {
        void refetch();
      }
  ...
  if (isPreviewMode() && preview) {
    return {
      gmCount: preview.gmCount,
      points: preview.points,
      lastGmAt: preview.lastGmAt,
      freeRemaining: preview.freeRemaining,
      deployCount: preview.deployCount,
      freeDeployAvailable: preview.freeDeployAvailable,
      totalGms: preview.totalGms,
      totalDeploys: preview.totalDeploys,
      gmFeeOnChain: preview.gmFeeOnChain,
      deployFeeOnChain: preview.deployFeeOnChain,
      minInterval: preview.minInterval,
      referralCount: preview.referralCount,
      airdropClaimed: preview.airdropClaimed,
      boostActiveUntil: preview.boostActiveUntil,
      freeBoostAvailable: preview.freeBoostAvailable,
      boostCount: preview.boostCount,
      boostFeeOnChain: preview.boostFeeOnChain,
      boostActive: preview.boostActive,
      refreshStats: preview.refreshStats,
      previewGm: preview.previewGm,
      previewDeploy: preview.previewDeploy,
    };
  }
