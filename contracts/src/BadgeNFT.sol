...
contract BadgeNFT {
    string public constant NAME = "DOZEN Badges";
    string public constant SYMBOL = "DOZEN";
  ...
  /// 21–24 Referral (2 / 5 / 10 / 20 friends)
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        ...
        if (category == CAT_GM) {
            return string(
                abi.encodePacked("DOZEN Badge: GM ", _uintToString(threshold))
            );
        }
        if (category == CAT_DEPLOY) {
            return string(
